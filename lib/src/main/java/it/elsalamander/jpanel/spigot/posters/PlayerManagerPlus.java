package it.elsalamander.jpanel.spigot.posters;
...
import it.elsalamander.jpanel.all.posters.PosterBase;
...
public class PlayerManagerPlus extends PosterBase{
	public PlayerManagerPlus(String path, PanelPlugin plugin){
		 ...
		 * plugin.getServer().getServicesManager().getRegistration(net.milkbowl.vault.
		 * permission.Permission.class);
		 * 
		 * if (permissionProvider != null) { permission =
		 * permissionProvider.getProvider(); } else { throw new
		 * NullPointerException("We couldn't get permissions from Vault! This is probably a bug!"
		 * ); }
		 */
	}
	...
	public Object getResponse(Request request, Response response){
		...
		if(requestJson.has("type")){
			requestJson.get("type").getAsString();
		}
		...
		if(requestJson.has("action")){
			...
			if(action.equals("list")){
				...
				if(plugin.getServer().getOfflinePlayer(playerUuid).isOnline()){
					...
					tinyPlayer.extras.put("groups",
							permission.getPlayerGroups(plugin.getServer().getWorlds().get(0).getName(), player));
				}
	...
	private List<String> getPlayerPermissions(Player player){
		List<String> perms = player.getEffectivePermissions().stream().map(PermissionAttachmentInfo::getPermission)
